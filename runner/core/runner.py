...
from runner.core.exceptions import ValidationFailure, StatusCodeValidationFailure, \
...
from runner.core.parser import build_url, parse_data, parse_variables_mapping, call_func
...
class ApiRunner(object):
    ...
    def __prepare_request_data(self, step: TStep):
        ...
        parsed_request_dict["headers"].setdefault(
            "User-Agent", cf.get_str("HEADERS", "User-Agent"),
        )
        parsed_request_dict["headers"].setdefault(
    ...
    def __run_step_request(self, step: TStep) -> StepData:
        ...
        step_data = StepData(sid=step.sid, name=step.name, description=step.description)
    ...
    def __run_step(self, step: TStep) -> StepData:
        ...
        logger.info("Run step begin: {} {} - {} >>>>>>".format(step.sid, step.name, step.description))
        ...
        logger.info("Run step end: {} {} - {} <<<<<<\n".format(step.sid, step.name, step.description))
        return step_data
    ...
    def run_testcase(self, testcase: TestCase) -> "ApiRunner":
        ...
        for step in self.__teststeps:
            ...
            try:
                if USE_ALLURE:
                    title = f"step{step.sid}: {step.name}"
                    if step.description:
        ...
        if skipped_count > 0 and USE_ALLURE:
            allure.dynamic.title("{}(Skipped:{})".format(testcase.config.description, skipped_count))
    ...
    def get_summary(self) -> TestCaseSummary:
        ...
        return TestCaseSummary(
            ...
            stat=TestStatusEnum.PASSED if self.success else TestStatusEnum.FAILED,
    ...
    def __prepare_logger(self) -> NoReturn:
        ...
        self.__log_path = self.__log_path or os.path.join(
            ...
            f"{time_str}-{self.__config.name}.run.log"
        )
        # 单独写Case日志到文件
        ...
        # 写日志到终端
        # self.__log_handlers.append(
        #     logger.add(sys.stdout, level=cf.get_str("LOGGER", "console_level"))
        # )
    ...
    def __prepare_run(self, param: Dict = None):
        ...
        logger.info("Start to run testcase: {}, ID: {}".format(self.__config.name, self.__case_id))
