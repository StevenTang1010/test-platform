...
@description:文件目录创建、文件创建、py用例文件创建
...
from runner.core import exceptions, __version__
from runner.core.compat import (
...
from runner.core.loader import (
    # load_folder_files,
    # load_test_file,
    load_testcase,
    load_testsuite,
...
from runner.core.response import uniform_validator
from runner.core.utils import merge_variables, is_support_multiprocessing
...
from runner.core import ApiRunner, Config, Step, RunRequest, RunTestCase
...
def make_config_chain_style(config: Dict) -> Text:
    ...
    if "department_name" in config:
        config_chain_style += f'.department_name("{config["department_name"]}")'
...
def make_testcase(testcase: Dict) -> Text:
    ...
    testcase_cls_name = config['name']
    testcase_description = config['description']
...
def make_testsuite(testsuite: Dict, dir_path: Text = None, idx: int = None) -> NoReturn:
    ...
    suite_safe_name = testsuite_config.get('name', '')
    testsuite_dir_name = "test_{}".format(suite_safe_name) if suite_safe_name else "test_{}".format(str(idx + 1).zfill(3))
    ...
    def make_testcase_ref(testcase_ref):
        ...
        if test_type in ['setup', 'teardown']:
            tc_py_name = "test_{}_{}.py".format(test_type, testcase_ref["name"])
        else:
            tc_py_name = "test_{}.py".format(testcase_ref["name"])
...
def __make(test_data: List[Dict], testcase_path: Text) -> NoReturn:
    ...
    for dept in test_data:
        dept_path = os.path.join(testcase_path, "test_"+dept.get('name', 'no_dept'))
        test_suite_list = dept.get('testsuites', [])
        for idx, test_suite in enumerate(test_suite_list):
            make_testsuite(test_suite, dept_path, idx)
