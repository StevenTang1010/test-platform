...
from runner.core.models import RequestData, ResponseData, ReqRespData, SessionData
from runner.core.utils import lower_dict_keys, omit_long_data
...
# 从响应对象获取到请求和响应信息
def get_req_resp_record(resp_obj: Response, logger=default_logger) -> ReqRespData:
    """
...
class HttpSession(requests.Session):
    ...
    def update_last_req_resp_record(self, resp_obj):
        ...
        self.data.req_resps.append(get_req_resp_record(resp_obj, self.logger))
    ...
    def request(self, method, url, name=None, **kwargs):
        ...
        try:
            ...
            self.logger.debug(f"client IP: {client_ip}, Port: {client_port}")
        except AttributeError as ex:
            self.logger.warning(f"failed to get client address info: {ex}")
        ...
        try:
            ...
            self.logger.debug(f"server IP: {server_ip}, Port: {server_port}")
        except AttributeError as ex:
            self.logger.warning(f"failed to get server address info: {ex}")
        ...
        self.data.req_resps = [
            get_req_resp_record(resp_obj, self.logger) for resp_obj in response_list
        ]
