...
from runner.core.loader import load_project_meta, convert_relative_project_root_dir
from runner.core.parser import parse_data
from runner.core.utils import sort_dict_by_custom_order
    ...
    for item in raw.split("."):
        if "-" in item:
            ...
            if len(raw_list) == 0:
                ...
                # sys.exit(1)
                raise Exception(f"Invalid jmespath: {raw}")
    ...
    if isinstance(extractors, List):
        ...
        for extractor in extractors:
            if not isinstance(extractor, Dict):
                ...
                # sys.exit(1)
                raise Exception(f"Invalid extractor: {extractors}")
...
def _ensure_step_attachment(step: Dict) -> Dict:
    test_dict = {
        ...
        "is_api_valid": step["is_api_valid"],
        "is_api_updated": step["is_api_updated"],
...
def ensure_testcase_v3(test_content: Dict) -> Dict:
    ...
    v3_content = {
        "id": test_content["id"],
        "config": test_content["config"],
        "department": test_content["department"],
        "test_type": test_content["test_type"],
        "tags": test_content["tags"],
        "teststeps": [],
        "setups": test_content["setups"],
        "teardowns": test_content["teardowns"],
    }

    if "teststeps" not in test_content:
        logger.error(f"Miss teststeps: {test_content}")
        # sys.exit(1)
        raise Exception(f"Miss teststeps: {test_content}")
    ...
    if not isinstance(test_content["teststeps"], list):
        ...
        # sys.exit(1)
        raise Exception(f'teststeps should be list type, got {type(test_content["teststeps"])}: {test_content["teststeps"]}')
...
def _generate_conftest_for_summary(args: List):
    for arg in args:
        ...
        # sys.exit(1)
        raise Exception(f"No valid test path specified! \nargs: {args}")
