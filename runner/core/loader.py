...
@description:
    1. 加载DICT转换为runner TestCase/TestSuite对象
    2. 预置函数加载成方法字典
    3. 路径加载
"""
import importlib
...
from runner.core import builtin, exceptions
from runner.core.models import TestCase, TestSuite, TestDept, ProjectMeta
# from apps.api_test.runner import customized
...
def load_testdept(test_dept: Dict) -> TestDept:
    """将部门用例集数据（字典）转成 TestDept对象"""
    try:
        # validate with pydantic TestDept model
        testdept_obj = TestDept.parse_obj(test_dept)
    except ValidationError as ex:
        err_msg = f"TestSuite ValidationError:\nerror: {ex}"
        raise exceptions.TestSuiteFormatError(err_msg)  # TODO
...
# 加载预置方法：customized.functions
def load_customized_functions() -> Dict[Text, Callable]:
    """load customized module functions"""
    # 避免循环导入
    from runner import customized

    return load_module_functions(customized)
