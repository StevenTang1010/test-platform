...
from runner.config import cf, root_dir
from runner.core.send_message import ReportQWChat
from runner.core.models import TestStatusEnum, ReportSummary
from runner.config import ReportAttr
from common.mysql import MysqlOperation
from common.sqlite import Sqlite3Operation
...
def session_fixture(request):
    ...
    try:
        if db_info.get('ENGINE') == 'django.db.backends.sqlite3':
            db_path = os.path.abspath(os.path.join(root_dir, '../db.sqlite3'))
    ...
    report_summary.time.start_at_iso_format = datetime.datetime.utcfromtimestamp(start_at).isoformat()
    ...
    for item in request.node.items:
        ...
        if testcase_summary.stat == TestStatusEnum.PASSED:
            report_summary.testcases_stat.passed += 1
        elif testcase_summary.stat == TestStatusEnum.FAILED:
            report_summary.testcases_stat.failed += 1
        elif testcase_summary.stat == TestStatusEnum.ERROR:
            report_summary.testcases_stat.error += 1
        elif testcase_summary.stat == TestStatusEnum.SKIPPED:
            report_summary.testcases_stat.skipped += 1
        else:
            logger.error("无效的测试结果状态！")
        if db:
            data = [(testcase_summary.stat.value, testcase_summary.case_id)]
        ...
        for step_data in step_datas:
            ...
            if step_data.stat == TestStatusEnum.PASSED:
                report_summary.teststeps_stat.passed += 1
            elif step_data.stat == TestStatusEnum.FAILED:
                report_summary.teststeps_stat.failed += 1
            elif step_data.stat == TestStatusEnum.ERROR:
                report_summary.teststeps_stat.error += 1
            elif step_data.stat == TestStatusEnum.SKIPPED:
                report_summary.teststeps_stat.skipped += 1
            else:
                logger.error("无效的测试结果状态！")
            if db:
                data = [(step_data.stat.value, step_data.step_id)]
                if db.__class__.__name__ == 'MysqlOperation':
    ...
    report_summary.stat = TestStatusEnum.PASSED if report_summary.success else TestStatusEnum.FAILED

    # 结果报告路径
    report_attr = ReportAttr(report_id)
    report_summary.allure_xml_path = report_attr.xml_report_path
    report_summary.html_report_path = report_attr.html_report_path
    # 插入测试结果到TestReport表
    ...
    report = {
        ...
        "broken_apis": json.dumps(report_summary.broken_apis),
