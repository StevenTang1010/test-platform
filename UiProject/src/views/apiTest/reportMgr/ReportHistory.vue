  ...
  <div class="main" style="padding-left:10px;">
    ...
    <el-col :span="24" class="toolbar" style="padding-bottom: 0;">
      ...
      <el-form :inline="true" :model="filters" :size="themeSize" @submit.native.prevent>
        <el-date-picker
          ...
          @change="fetchData"
        />
        <el-select v-model="filters.id" :size="themeSize" placeholder="ID" value-key="id" multiple filterable allow-create clearable @keyup.enter.native="fetchData">
          <el-option v-for="item in []" :key="item.value" :label="item.label" :value="item.value.replace(/[^\d]/g,'')" />
        </el-select>
        <el-select v-model="filters.env" value-key="id" placeholder="环境筛选" :size="themeSize" clearable>
          <el-option v-for="item in globalEnv_options" :key="item.id" :label="'环境配置:'+item.name" :value="item" />
        </el-select>
        <el-select v-model="filters.build_type" value-key="id" placeholder="构建类型筛选" :size="themeSize" clearable>
    ...
    >
      ...
      <el-table-column prop="broken_apis" label="阻塞" min-width="70" sortable>
        <template slot-scope="scope">
          <el-button :size="themeSize" type="text" style="color: #ee0505" @click="handleBrokenApis(scope.row)">
      ...
      <el-table-column v-if="isSuperuser" label="操作">
        <template slot-scope="scope">
          <el-button type="danger" :size="themeSize" @click="handleDel(scope.$index, scope.row)">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    >
      <div class="demo-drawer__content">
        <el-table :data="broken_apis" style="width: 100%" :size="themeSize">
          ...
          <el-table-column label="desc" prop="api_desc" show-overflow-tooltip min-width="200">
            <template slot-scope="scope">
              <el-popover
                placement="right"
                width="500"
...
import {
  ...
  getJenkinsAllure
} from '@/api/apiTest/test_report'
import BuildStatus from '@/views/apiTest/components/BuildStatus'
...
export default {
  ...
  data() {
    return {
      ...
      isSuperuser: false,
      isQaLab: process.env.VUE_APP_BASE_API.indexOf('qalab.dustess.com') !== -1,
