<template>
  <div class="main" style="padding:10px;">
    ...
    >
      ...
      <el-table-column prop="value" label="内容" sortable show-overflow-tooltip width="300" />
      <el-table-column prop="description" label="描述" sortable show-overflow-tooltip width="300" />
...
import {
  addGlobalConst,
  bulkDeleteGlobalConst,
  deleteGlobalConst,
  getGlobalConstList,
  updateGlobalConst
} from '@/api/apiTest/global_const'
...
export default {
  ...
  methods: {
    ...
    fetchData() {
      ...
      getGlobalConstList(params).then(response => {
    ...
    handleChangeStatus: function(index, row) {
      if (row.status) {
        // 禁用
        updateGlobalConst(row.id, { status: false }).then(response => {
        ...
        // 启用
        updateGlobalConst(row.id, { status: true }).then(response => {
          const { msg, code } = response
          if (code === 2) {
            this.$message({
              message: '启用成功',
      ...
      this.$refs.editForm.validate((valid) => {
        if (valid) {
          this.$confirm('确认提交吗？', '提示', {}).then(() => {
            ...
            updateGlobalConst(Number(this.editForm.id), params).then(response => {
              const { msg, code } = response
      ...
      this.$refs.addForm.validate((valid) => {
        if (valid) {
          this.$confirm('确认提交吗？', '提示', {}).then(() => {
            ...
            addGlobalConst(params).then(response => {
              const { msg, code } = response
    ...
    handleDel: function(index, row) {
      this.$confirm('确认删除该记录吗?', '提示', {
        ...
        // NProgress.start();
        deleteGlobalConst(row.id).then(response => {
          const { msg, code } = response
    ...
    bulkRemove: function() {
      ...
      this.$confirm('确认删除选中记录吗？', '提示', {
        ...
        bulkDeleteGlobalConst(params).then(response => {
          const { code, msg } = response
          if (code === 2) {
            this.$message({
              message: '删除成功',
