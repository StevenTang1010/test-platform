<template>
  <div class="main" style="padding:10px;">
    ...
    >
      ...
      <el-table-column prop="safe_name" label="标识" min-width="10%" sortable />
      <el-table-column prop="leader" label="负责人" min-width="8%" sortable>
        <template slot-scope="scope">
          <span>{{ scope.row.leader.name }}({{ scope.row.leader.role }})</span>
        </template>
      </el-table-column>
      <el-table-column prop="members" label="成员" min-width="8%">
        <template slot-scope="scope">
          <el-popover
            placement="right"
            width="800"
          ...
          >
            <el-card class="box-card" shadow="never">
              <div slot="header" class="clearfix">成员列表</div>
              <el-table
                :data="scope.row.members"
                row-key="id"
                :default-sort="{prop: 'user.username', order: 'ascending'}"
                style="width: 100%"
                :size="themeSize"
              >
                <el-table-column label="名称" prop="user.username" sortable show-overflow-tooltip />
                <el-table-column label="角色" prop="role" sortable show-overflow-tooltip />
                <el-table-column label="email" prop="user.email" sortable show-overflow-tooltip />
              </el-table>
            </el-card>
    ...
    <!--编辑界面-->
    <el-dialog title="编辑" :visible.sync="editFormVisible" :close-on-click-modal="false" style="width: 100%;">
      <el-form ref="editForm" :size="themeSize" :model="editForm" label-width="120px" :rules="editFormRules">
        ...
        <el-form-item label="成员" prop="leader">
          <el-transfer
            v-model="editForm.members"
            filterable
            :filter-method="filterMethod"
            filter-placeholder="请输入user拼音"
            :data="userList"
          />
        </el-form-item>
        <el-form-item label="描述" prop="description">
          <el-input v-model="editForm.description" type="textarea" :rows="1" />
        </el-form-item>
      </el-form>
...
export default {
  ...
  data() {
    return {
      ...
      tableConfig: {
        isLoading: false,
        height: window.innerHeight - 180 // 下面剩余多少空白部分（即最下面距离底部有多少距离）
      },
      editLoading: false,
      addLoading: false,
      delLoading: false,
